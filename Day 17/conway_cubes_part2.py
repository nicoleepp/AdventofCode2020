    ...
    for curr_x in (x-1, x, x+1):
        for curr_y in (y-1, y, y+1):
            for curr_z in (z-1, z, z+1):
                for curr_w in (w-1, w, w+1):

                    if curr_x == x and curr_y == y and curr_z == z and curr_w == w:
                        continue

                    state = cubes.get((curr_x, curr_y, curr_z, curr_w), None)
            ...
            for curr_y in (y-1, y, y+1):
                for curr_z in (z-1, z, z+1):
                    for curr_w in (w-1, w, w+1):
                        ...
                        state = cubes.get((curr_x, curr_y, curr_z, curr_w), None)
                        if state is None:
                            new_cubes[(curr_x, curr_y, curr_z, curr_w)] = INACTIVE_STATE
                        else:
                            new_cubes[(curr_x, curr_y, curr_z, curr_w)] = state
    return new_cubes
